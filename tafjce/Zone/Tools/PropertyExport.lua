...
function processPropDef(input, output)
	...
	--local defstring = "__PROP_DEF_"..MagicInfo.."_"..os.time()
	--local outString = "#ifndef "..defstring.."\n"
	local outString = "$cfile \""..input.."\"\n"
	--outString = outString.."#define "..defstring.."\n"
	--outString = outString.."//time:"..os.date().."\n"
	--outString = outString.."//tolua_begin\n"
	outString = outString.."enum\n{\n"
	...
	while true do
		...
		if pos == 0 then
			...
			string.gsub(lineData, "^PROP_SIMPLEDEFIMP[\(](.+)[\)]", createParamFun(1))
			string.gsub(lineData, "^PROP_DEFIMP[\(](.+)[\)]", createParamFun(1, 3))
			if string.len(keyStr) > 0 then
				--outString =outString..keyStr..","..keyValue..","..keyType.."\r\n"
				outString = outString.."\t"..keyStr.."="..keyValue..",\n"
			end
		end
	end
	outString = outString.."\n};\n"
	--outString = outString.."//tolua_end\n"
	--outString = outString.."#endif"
